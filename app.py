...
def blank_entry_table():
    return pd.DataFrame([
        {"Machine Name": "", "Manufacturing Order": "", "Operation Number": "", "Completed Quantity": 0, "Rejected Quantity": 0}
...
with st.sidebar:
    ...
    st.caption("The app does not deliberately persist report data. Use the JSON and PDF download buttons to keep your records on the computer you are using.")
...
if workflow == "Daily Entry":
    ...
    c1, c2 = st.columns(2)
    report_date = c1.date_input("Production date", value=st.session_state.daily_date)
    product_name = c2.text_input("Focused product", value=st.session_state.daily_product, placeholder="e.g. AT7701")

    uploaded_daily = st.file_uploader("Load an existing daily JSON to continue/edit", type=["json"], key="edit_daily_json")
    if uploaded_daily is not None:
        try:
            payload = load_daily_payload(uploaded_daily.getvalue())
        except Exception as exc:
            st.error(f"Could not load this daily file: {exc}")
        else:
            load_key = f"{uploaded_daily.name}:{len(uploaded_daily.getvalue())}"
            if st.session_state.get("loaded_daily_key") != load_key:
                rows = []
                for r in payload["records"]:
                    rows.append({
                        "Machine Name": r.get("Machine Name", ""),
                        "Manufacturing Order": r.get("Manufacturing Order", ""),
                        "Operation Number": r.get("Operation Number", ""),
                        "Completed Quantity": r.get("Completed Quantity", 0),
                        "Rejected Quantity": r.get("Rejected Quantity", 0),
                    })
                st.session_state.daily_editor = pd.DataFrame(rows, columns=ENTRY_COLUMNS) if rows else blank_entry_table()
    ...
    if not preview.empty:
        preview["Completed Quantity"] = pd.to_numeric(preview["Completed Quantity"], errors="coerce").fillna(0)
        preview["Rejected Quantity"] = pd.to_numeric(preview["Rejected Quantity"], errors="coerce").fillna(0)
    ...
    for _, row in edited.iterrows():
        if all(str(row.get(c, "")).strip() in ["", "0", "0.0"] for c in ["Machine Name", "Manufacturing Order", "Operation Number", "Completed Quantity", "Rejected Quantity"]):
            continue
        completed = float(pd.to_numeric(row.get("Completed Quantity", 0), errors="coerce") or 0)
        rejected = float(pd.to_numeric(row.get("Rejected Quantity", 0), errors="coerce") or 0)
    ...
    if clean_rows and not invalid:
        ...
        st.caption("Keep these daily JSON files in date order on your computer. They are the source files used to build weekly, monthly and yearly reports.")
    elif not clean_rows:
    ...
    st.write("Upload the daily JSON files you previously downloaded. The app sorts them chronologically by the date stored inside each file.")
    ...
    for f in uploads:
        try:
            payloads.append(load_daily_payload(f.getvalue()))
        except Exception as exc:
    ...
    st.caption("Use this only for the older weekly Excel workbook format. New reporting should use Daily Entry + JSON files.")
    ...
    try:
        pdf = generate_pdf(filtered, selected_product)
    except Exception as exc:
